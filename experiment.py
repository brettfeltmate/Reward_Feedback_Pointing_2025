...
from klibs.KLUserInterface import key_pressed, pump, ui_request, get_clicks, mouse_pos
...
class reward_feedback_pointing_2025(klibs.Experiment):
    ...
    def setup(self):
        ...
        # Define stimuli
        self.stimuli = {
            "fix": kld.FixationCross(size=self.fix_w, thickness=self.thick, fill=WHITE),
            "rect": kld.Rectangle(
                width=self.rect_w,
                height=self.rect_he,
        ...
        if (
            ...
            self.conditions = [cond for _ in range(3) for cond in self.conditions]
    ...
    def block(self):
        ...
        instrux = (
            ...
            "\n\n"
            f"Block condition: {self.condition}"
            "\n\n"
    ...
    def trial_prep(self):
        ...
        self.bs.add_boundaries(
            [
                CircleBoundary("penalty", self.positions["penalty"], self.circle_d / 2),
                CircleBoundary("reward", self.positions["reward"], self.circle_d / 2),
            ]
        )
